...
const schema = yup.object({
  fullName: yup
    ...
    .matches(/^[A-Za-z\u0600-\u06FF\s]{3,}$/, "Name must contain letters only"),
  ...
  city: yup
    ...
    .matches(/^[A-Za-z\u0600-\u06FF\s]{3,}$/, "City must match letters only"),
  area: yup
    .string()
    .required("Area is required")
    .matches(
      /^[A-Za-z\u0600-\u06FF0-9\s]{3,}$/,
      "Symbols are not allowed and must be at least 3 characters"
    ),
  address: yup
    .string()
    ...
    .matches(
      /^[A-Za-z\u0600-\u06FF0-9\s]{5,}$/,
      "Symbols are not allowed and must be at least 5 characters"
    ),
  floor: yup
    .string()
    .optional()
    .test("is-valid-floor", "Floor must be a number", (value) => {
      if (!value || value.trim() === "") return true; // طالما الحقل فاضي، اعتبره صالح
      return /^\d+$/.test(value) && parseInt(value) < 100;
    }),
});
